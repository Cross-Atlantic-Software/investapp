...
function ProductRow({ item, sectorsMap }: { item: ProductItem; sectorsMap: Record<number, string>; onWishlist?: (id: string) => void }) {
  ...
  return (
    <article className="w-full rounded-xl bg-themeTealWhite p-2 md:p-3 min-h-[140px]">
      <div className="flex flex-col gap-3 xl:flex-row md:items-center md:justify-between">
        <div className="flex gap-3 md:gap-4 items-center">
          ...
          <div className="min-w-0 w-full flex-1">
            ...
            <p className="mt-1.5 text-sm text-themeTealLight line-clamp-4 md:line-clamp-3 leading-relaxed max-w-full">
        ...
        <div className="w-full md:w-auto md:min-w-[340px] xl:min-w-[460px]">
          <div className="hidden md:grid grid-cols-3 bg-themeTeal px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-white">
            <div>Price per share</div><div>Price Change Period</div><div>Valuation (in Cr.)</div>
          </div>
          <div className="hidden md:grid grid-cols-3 items-center gap-1 bg-white px-2 py-1.5 text-xs font-semibold text-themeTeal">
