...
export async function GET(
  ...
  try {
    ...
    const url = `${API_BASE_URL}/api/admin/stocks/${id}/performance-pdfs?${queryParams.toString()}`;
...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/admin/stocks/${id}/performance-pdfs`, {
      method: 'POST',
      headers: {
        'token': token,
      },
