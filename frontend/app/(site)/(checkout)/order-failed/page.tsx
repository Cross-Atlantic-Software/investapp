// app/(site)/(checkout)/order-failed/page.tsx
"use client";
        ...
        <div className="mx-auto max-w-4xl rounded bg-white p-6 sm:p-10">
          ...
          <div className="mt-8 rounded bg-red-50 p-6 sm:p-10">
            <div className="flex flex-col items-center">
              <span className="grid h-16 w-16 place-items-center rounded-full bg-red-600">
                <X className="h-12 w-12 text-white" />
              </span>
              <p className="mt-4 text-lg font-semibold text-red-700">Payment Failed!</p>
            </div>

            <div className="mt-6 rounded border border-red-700 bg-white">
              <div className="grid grid-cols-1 gap-y-4 p-5">
                <KV k="Order ID" v={orderId} />
                <KV k="Payment ID" v={paymentId} />
                <KV k="Amount Attempted" v={amount} />
                <KV k="Payment Method" v={method} />
              </div>
            </div>

            <div className="mt-6 text-center">
              <p className="text-red-700">Your payment failed. The order is not active.</p>
              <p className="text-sm text-red-700">You can retry payment or choose another method.</p>
            </div>

            <div className="mt-6">
              <Link
                href="/checkout/payment"
                className="block w-full rounded bg-red-700 px-6 py-3 text-center font-medium text-white transition hover:bg-red-800"
              >
                Retry Payment
              </Link>
            </div>
