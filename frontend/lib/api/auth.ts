...
export const authApi = {
  async register(data: RegisterRequest): Promise<AuthResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/backend/api/auth/register`, {
        method: 'POST',
        headers: {
  ...
  async verifyEmail(data: VerifyEmailRequest): Promise<AuthResponse> {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/backend/api/auth/verify-email`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          email: data.email,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          const responseWithoutAuth = await fetch(`${API_BASE_URL}/backend/api/auth/complete-profile`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  async googleAuth(): Promise<{ authUrl: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}/backend/api/auth/google`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async googleTokenVerify(data: GoogleAuthRequest): Promise<AuthResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/backend/api/auth/google/verify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
